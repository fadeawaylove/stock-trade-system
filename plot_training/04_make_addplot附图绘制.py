...
add_plot = [
    ...
    mpf.make_addplot(daily[["High", "Low"]]),
    # 添加附图，使用panel参数
    mpf.make_addplot(daily["Volume"], type="bar", panel=1, color='g', secondary_y='auto')
]

# panel_ratios设置主图和附图的比例（两个附图panel_ratios=(1, 1，0.5)）
mpf.plot(daily, type='candle', panel_ratios=(1, 1), addplot=add_plot)
